...
<body>
	...
	<div class="outer" >
		...
		<script>
			...
			$(function(){
				// 게시글 정보인 b에서 가져온 카테고리명과 지금 select에 option으로 등록되어있는 
				// 목록중 일치하는 것을 찾아 선택시키기
				
				var cName = "${b.categoryNo}"; // 선택되어있던 카테고리명
				/* console.log(cName) */
				
				// 옵션들의 텍스트 중에서 cName과 일치한 텍스트요소를 선택해 놓을 수 있도록 처리
				// 옵션태그 찾기
				...
				$("#update-area option").each(function(){
					...
					// option태그의 text요소와 cName비교
					if($(this).text()==cName){
						$(this).attr("selected",true);
						// 요소를 찾아 처리했으면 나머지 순회는 필요없다. 벗어나기
						return false; // 단순 반복문 아니기때문에 break; 아니고 return false로 함수 벗어나기
					}
					...
					<td colspan="3">
						...
						<c:if test="${at != null }">
							...
							<!-- 게시글에 첨부파일이 있었던 경우 해당 첨부파일 정보를 등록한 DB에 있는 정보에서 수정을 해야한다 
								 때문에 해당 데이터 식별자용으로 fileNo를 전달하고 서버에 업로드 된 파일을 삭제하기 위해
								 changeName도 전달하기
							-->
