...
<body>
	...
	<div class='outer' align="center">
		...
		<!-- 로그인한 회원이 작성자 본인이라면 또는 관리자라면 수정/삭제 버튼 활성화버튼 보이도록 -->
		<c:if
			test="${loginUser.userId eq b.boardWriter or loginUser.userId eq 'admin'}">
			<div align="center">
				<button type="button"
					...
					<c:choose>
						...
						<c:otherwise>
							<tr>
								...
								<td>
									<button disabled="disabled">댓글작성</button>
								</td>
							</tr>
						</c:otherwise>
					</c:choose>
		...
		<script>
			...
			function insertReply() {
				...
				/*
					...
					완료시 alert으로 댓글작성 성공 메세지 보여주며 작성란 비워주기
					실패시 alert으로 댓글작서 실패 메세지 보여주기
			...
			$(function() {
				...
				// 문서 dom요소 만들어지면 댓글 목록 조회
				replyList();
				
		<%-- 삭제하기 버튼을 누르면 정말 삭제하시겠습니까? 확인 취소가 나오는 confirm을 이용하여
								 확인을 눌렀을때 삭제 요청이 되도록 처리하기
								 매핑주소 : delete.bo
								 메소드명 : deleteBoard()
								 컨트롤러명 : BoardDeleteController()
								 
								 함수를 이용하여 post방식 form요청이 될 수 있도록 처리할 것
								 
								 삭제시 DML - DELETE를 사용한다면 업로드 파일도 삭제
								 삭제시 DML - UPDATE를 사용한다면 업로드 파일은 보류
							 --%>
			$("#deleteBtn").click(function() {
							if (confirm("정말 삭제하시겠습니까?")) {
								...
								// 만든 form을 문서 body에 넣고 submit 실행
								$(body).append(form);
								form.submit();
