...
public class MemberLoginController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// POST방식의 경우 전달값에 한글이 포함되어 있다며 인코딩 처리 할 것
		request.setCharacterEncoding("UTF-8");
		
		// 전달받은 데이터는 request의 parameter영역에 담겨온다
...
//		System.out.println(loginUser);
		 ...
		 * 					한번 담은 데이터는 직접지우기 전까지 서버가 멈추기 전까지 유지된다.
		 * 
		 * 2) session : session에 담은 데이터는 모든 jsp와 servlet에서 접근할 수 있다
		 * 				한번 담은 데이터는 직접지우기 전까지, 서버가 멈추기 전까지, 브라우저가 종료되기 전까지 접근가능
		 * 
		...
		// session에 담아두는 이유: 다른페이지를 가도 로그인이 유지될 수 있도록
