...
public class MemberInsertController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 취미 같은 경우 문자열배열로 되어있기 때문에 데이터베이스에 그대로 등록할 수 없다.
		// 문자열로 만들어서 등록하기 
		// (이때 체크 안하면 null이 나오기 때문에 조건처리로 null이 아닌 경우만 문자열 만들기)
		...
		// 추출한 데이터들을 service-dao-db에 전달하기 위해서 회원정보객체에 담아주기
		...
		HttpSession session = request.getSession(); // 세션 받아보기
		...
		if(result>0) { // 성공시 화면
			...
			// 회원가입을 하지 못했으니 에러페이지로 요청 위임하며 전달하기
