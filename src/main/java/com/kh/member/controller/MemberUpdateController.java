...
public class MemberUpdateController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String interestStr = "";
		if(interest != null) { // null이 아니면 문자열로 합치기
			interestStr = String.join(",", interest);
		}
		
		// 추출한 데이터 객체에 담아서 service에 전달 및 요청처리
		// 메소드명 UpdateMember
		// 처리된 결과에 따라서 성공시 정보수정 성공! 메시지를 세션에 담고
		// 마이페이지로 재요청
		...
		if(result>0) {
			...
			// 정보수정이 성공했으면 마이페이지를 다시 보여주기 전에 
			// 세션에 담겨있는 기존정보를 변경된 정보로 갱신해야한다.
			// 로그인 유저 정보 다시 조회해오기
			// 기존에 로그인할때 사용했던 메소드 이용하여 로그인정보 가져오기
			// 세션에 있는 기존 정보 중 비밀번호 추출하여 사용하기
			...
			// 조회해온 회원정보 세션에 담아주기 (기존의 키값에 데이터 담으면 덮어쓰기 (갱신))
...
//			response.sendRedirect(request.getContextPath()+"/views/member/myPage.jsp");
			...
			// 실패시 정보수정 실패! 메시지를 request에 담고
			// 에러 페이지로 위임(포워딩)
			request.setAttribute("errorMsg", "정보수정 실패!");
