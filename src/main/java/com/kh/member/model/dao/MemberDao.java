...
public class MemberDao {
	...
	public Member loginMember(Connection conn, String userId, String userPwd) {
		...
		PreparedStatement pstmt =null; // sql구문 준비 및 실행처리결과 받아낼 객체 선언
		Member m = null; // 조회된 결과가 있다면 담아갈 회원 정보객체
		
		
		// prop 객체에 읽어온 파일 데이터 중 loginMember라는 키값의 sql구문 추출하기
		String sql = prop.getProperty("loginMember");
		
		
		try {
			// 미완성 sql구문 전달하며 pstmt객체 생성
			...
			// 따라서 있는지 없는지 조건처리만 하면 됨 (반복할 필요 없음)
			
			// rset.next() : 커서가 다음행으로 이동하며 접근할 있다면 true 없으면 false 반환
			if(rset.next()) { // 접근할 행이 있을 때
				// 접근된 행에 있는 데이터를 추출하여 Member객체에 담아서 반환하기
	...
	public int insertMember(Connection conn, Member m) {
		...
		// C(insert) - DML
		// DML구문시 필요한 준비물
		PreparedStatement pstmt = null; // sql문 전달 및 실행 객체 선언
		...
		try {
			...
			pstmt.setString(7, m.getInterset());
			
			// 완성됐으면 실행 및 처리결과받기
			// DML구문 처리된 행수를 반환한다.
			result = pstmt.executeUpdate(); // dml구문은 executeUpate()메소드 사용
	...
	public int updateMember(Connection conn, Member m) {
		...
		String sql = prop.getProperty("updateMember");
		 ...
		 try {
			...
			pstmt.setString(5, m.getInterset());
	...
	public int updatePwd(Connection conn, HashMap<String, String> map) {
		...
		String sql = prop.getProperty("updatePwd"); // sql 구문준비
	...
	public String idCheck(Connection conn, String id) {
		...
		try {
			...
			if(rset.next()) { // 어차피 하나의 값이기 때문에 while대신 if로 조건처리해주면된다
				
				checkId = rset.getString("user_id");
			}
			
		 // * boolean flag =false; 를 만들어서 전달값 전달해주면 간단함 (flag=rset.next())  
