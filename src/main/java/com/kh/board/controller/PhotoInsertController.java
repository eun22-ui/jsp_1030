...
public class PhotoInsertController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			// Attachment 정보 추출
			// 첨부파일의 경우 대표이미지에 required속성을 작성했기 때문에 무조건 1개 이상 전달된다
			// 여러개의 파일정보가 전달된다면 반복문을 이용하여 처리하기
			// Attachment 객체 여러개를 담아야하니 list 준비 
			ArrayList<Attachment>  atList = new ArrayList<>();
			
			// 전달받은 키값은 file1,file2,file3,file4 형태이기 때문에 file + 증가 숫자로 키 값 접근
			for(int i=1; i<=4; i++) {
				// i값 이용하기 4번반복
				String key = "file"+i;
				...
				if(multirequest.getOriginalFileName(key) != null) {
					// 원본파일명에 접근했을때 null이 아닐때, 즉 해당 키값으로 전달된 파일이 있을때
